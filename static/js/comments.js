...
for (let button of editButtons) {
    button.addEventListener('click', (e) => {
        let commentId = e.target.closest('button').getAttribute('data-comment_id');
        let commentContent = document.getElementById(`comment_${commentId}`).innerText;
        commentText.value = commentContent;
        submitButton.innerText = 'Update';
        commentForm.setAttribute('action', `/reports/comments/${commentId}/edit`);
    });
}
...
for (let button of deleteButtons) {
    button.addEventListener('click', (e) => {
        let commentId = e.target.closest('button').getAttribute('data-comment_id');
        deleteConfirm.href = `/reports/comments/${commentId}/delete`;
        deleteModal.show();
    });
}
